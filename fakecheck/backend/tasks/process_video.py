...
async def send_analysis_results(video: Video, analysis_dict: dict, ai_probability: float = 0.0):
    ...
    try:
        if video.status == "done":
            ...
            verdict_text = "Nicht bewertbar (Unterhaltung/Musik)" if verdict in ["music", "joke"] else "Wahrscheinlich Fake" if verdict == "likely_fake" else "Unsicher" if verdict == "uncertain" else "Wahrscheinlich Echt"
            
            message = f"{verdict_emoji} Analyse: {verdict_text} {verdict_emoji}\n\n"
            ...
            if valid_sources:
                message += "🔗 **Quellen zum Nachlesen:**\n"
                for src in valid_sources:
                    message += f"• {src}\n\n"
                    
            ai_percentage = int(ai_probability * 100)
            if ai_percentage >= 70:
                message += f"🚨 **Achtung: Dieser Beitrag ist zu {ai_percentage}% KI generiert!** 🚨\n\n"
                
            if verdict in ["joke", "music"]:
                message += "Fragen dazu?\n\nMöchtest du trotzdem, dass ich den Beitrag überprüfe?"
            else:
                message += "Fragen dazu?"
            ...
            message = f"❌ Fehler bei der Analyse\n\nLeider gab es ein Problem:\n{video.error_message}"
            async with httpx.AsyncClient() as client:
                await client.post(
                    f"https://api.telegram.org/bot{settings.TELEGRAM_BOT_TOKEN}/sendMessage",
                    json={
