...
async def check_ai_probability(video_path: str) -> float:
    """
    Submits the video to Sightengine genai model.
    ...
    try:
        async with httpx.AsyncClient() as client:
            with open(video_path, 'rb') as f:
                ...
                res = await client.post(url, files=files, data=data, timeout=120)
    ...
    if data_json:
        if data_json.get("status") == "success" and "data" in data_json and "frames" in data_json["data"]:
            frames = data_json["data"]["frames"]
            max_ai = 0.0
            for frame in frames:
            ...
            logger.warning(f"Sightengine unexpected response: {data_json}")
            return 0.0
