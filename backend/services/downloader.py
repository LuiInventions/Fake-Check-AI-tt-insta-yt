            ...
            if "instagram.com" in target_url:
                try:
                    ...
                    if res.status_code == 429:
                        raise Exception("RapidAPI Error 429: Too Many Requests for 'Instagram Looter'.")
                    elif res.status_code == 403:
                        raise Exception("RapidAPI Error 403: Subscription for 'Instagram Looter' missing.")
                    ...
                    if data.get("status") and "data" in data and "medias" in data["data"]:
                        ...
                        if video_urls:
                            ...
                            raise Exception("Instagram Looter API: No media URL found in JSON.")
                    else:
                        raise Exception(f"RapidAPI Unknown JSON: {str(data)[:300]}")
                        
                    logger.info("Successfully extracted Instagram raw video URL via Instagram Looter")
                except Exception as ie:
                    if "RapidAPI Error" in str(ie) or "RapidAPI JSON" in str(ie):
