...
# Safer regex to prevent ReDoS
URL_REGEX = re.compile(r'(https?://[^\s]{10,500})', re.IGNORECASE | re.DOTALL)

def send_telegram_message(chat_id: str, text: str):
    """Helper function to send Telegram messages without exposing token in logs"""
    return httpx.post(
...
async def telegram_webhook(request: Request, background_tasks: BackgroundTasks, db: AsyncSession = Depends(get_db)):
    ...
    # DO NOT log token-containing data
    logger.info(f"Telegram webhook received from chat")
    ...
    if len(text) > 10000:
        async with httpx.AsyncClient() as client:
            await client.post(
                f"https://api.telegram.org/bot{settings.TELEGRAM_BOT_TOKEN}/sendMessage",
                json={"chat_id": chat_id, "text": "Message too long. Please keep it shorter."}
            )
        return {"status": "text_too_long"}

    # Extract URL from text (with ReDoS-safe regex)
        ...
        if not latest_video or latest_video.status != "done":
            async with httpx.AsyncClient() as client:
                await client.post(
                    ...
                    json={
                        "chat_id": chat_id,
                        "text": "Please send me a link to a TikTok, Insta, or YouTube video first so I can analyze it."
                    }
                )
    ...
    # Validate URL (additional check, should also be validated by VideoAnalyzeRequest)
    if not url.startswith(('http://', 'https://')):
        async with httpx.AsyncClient() as client:
            await client.post(
                f"https://api.telegram.org/bot{settings.TELEGRAM_BOT_TOKEN}/sendMessage",
                json={"chat_id": chat_id, "text": "Invalid URL. Please send a valid link."}
            )
    ...
    logger.info(f"Video processing started for Telegram chat {chat_id}")

    # Send processing message
    async with httpx.AsyncClient() as client:
        await client.post(
            f"https://api.telegram.org/bot{settings.TELEGRAM_BOT_TOKEN}/sendMessage",
            json={"chat_id": chat_id, "text": "Checking video... ⏳"}
        )
        ...
        try:
            ...
            if video and video.analysis_result:
                ...
                if verdict in ["joke", "music"]:
                    ...
                    affirmative_words = ["yes", "please", "do it", "anyway", "check", "analyze", "go ahead", "sure"]
            ...
            async with httpx.AsyncClient() as client:
                await client.post(
                    f"https://api.telegram.org/bot{settings.TELEGRAM_BOT_TOKEN}/sendMessage",
                    json={"chat_id": chat_id, "text": "Sorry, there was a problem processing your question."}
                )
